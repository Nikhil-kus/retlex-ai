...
/**
 * HistoryPageContent — extracted from src/app/history/page.tsx
 * Accepts shop/shopId as props instead of fetching /api/shop.
 */
...
export default function HistoryPageContent({ shop, shopId }: Props) {
  ...
  useEffect(() => {
    fetch(`/api/bills?shopId=${shopId}`)
      .then(r => r.json())
      .then(data => { if (Array.isArray(data)) setBills(data); })
  ...
  // Bill detail link — use new multi-tenant route if available, fallback to legacy
  const billHref = (id: string) => `/${shopId}/history/${id}`;
    ...
    <div className="p-6 md:p-10 max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">View and manage all past bills for {shop.name}.</p>
        </div>
      </div>
                ...
                filteredBills.map((b) => (
                  <tr key={b.id} className="hover:bg-slate-50/50">
                    ...
                    <td className="px-6 py-4 text-right">
                      <Link href={billHref(b.id)} className="inline-flex items-center gap-1 px-3 py-1.5 bg-indigo-50 text-indigo-600 hover:bg-indigo-100 rounded-lg transition font-medium text-xs">
                        <Eye size={14} /> View
                      </Link>
                    </td>
                  </tr>
