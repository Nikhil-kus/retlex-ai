...
/**
 * UnpaidPageContent — extracted from src/app/unpaid/page.tsx
...
export default function UnpaidPageContent({ shop, shopId }: Props) {
  ...
  useEffect(() => {
    fetch(`/api/bills?shopId=${shopId}`)
      .then(r => r.json())
      .then(data => {
        if (Array.isArray(data)) setBills(data.filter((b: Bill) => b.status === 'UNPAID'));
      })
    ...
    <div className="p-6 md:p-10 max-w-5xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1">Manage pending payments for {shop.name}.</p>
        </div>
            ...
            <div key={b.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex flex-col justify-between hover:border-rose-200 transition">
              ...
              <div className="mt-auto">
                ...
                <div className="flex gap-2">
                  ...
                  <Link href={billHref(b.id)} className="bg-slate-50 text-slate-600 hover:bg-slate-100 p-2 rounded-lg transition">
                    <Eye size={20} />
                  </Link>
                </div>
              </div>
