/**
 * src/lib/alias-utils.ts
 * ─────────────────────────────────────────────────────────────────────────────
 * Utility for generating local Hindi name aliases for products using Gemini.
 * ─────────────────────────────────────────────────────────────────────────────
 */

export async function generateLocalAliases(name: string, localName: string | null): Promise<string[]> {
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) {
    console.warn("GEMINI_API_KEY is not defined. Skipping alias generation.");
    return [];
  }
...
Product Hindi Name: "${localName || ''}"`;
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`Gemini API error generating aliases: ${res.statusText}`);
      return [];
    }
    ...
    console.error("Error in generateLocalAliases:", error);
    return [];
  }
}
