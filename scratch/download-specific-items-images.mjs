...
(async () => {
  ...
  for (const shopId of shopIds) {
    ...
    const products = snap.docs.map(d => ({ id: d.id, ...d.data(), shopId }));
    allProducts.push(...products);
  }

  const targetNames = [
    'golden rice', 'hmt rice', 'parboiled rice',
    'bubble daliya', 'chhola chhana', 'magic masala', 'mahara besan', 
    'moong dal', 'roastman chana', 'royal chana', 'silvercoin daliya'
  ];

  const matchedProducts = allProducts.filter(p => {
    if (!p.name) return false;
    const n = p.name.toLowerCase();
    return targetNames.some(target => n.includes(target));
  });

  const uniqueNames = [...new Set(matchedProducts.map(p => p.name))];
  console.log(`Found ${matchedProducts.length} total matched products, ${uniqueNames.length} unique.`);
  ...
  for (let i = 0; i < uniqueNames.length; i++) {
    ...
    const representativeProduct = matchedProducts.find(p => p.name === name);
    ...
    // Add context to search query depending on the item
    let queryStr = `${clean} product packaging India`;
    if (clean.toLowerCase().includes('rice') || clean.toLowerCase().includes('dal') || clean.toLowerCase().includes('chana') || clean.toLowerCase().includes('besan') || clean.toLowerCase().includes('daliya')) {
