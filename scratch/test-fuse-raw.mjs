...
async function run() {
  ...
  searchNames.forEach(queryName => {
    ...
    results.slice(0, 10).forEach((r, idx) => {
      console.log(`  ${idx + 1}. Name: ${r.item.name} | Local: ${r.item.localName} | Aliases: ${JSON.stringify(r.item.localAliases)} | Score: ${r.score.toFixed(4)} | Unit: ${r.item.baseUnit} | Weight: ${r.item.packetWeight}`);
    });
  });
