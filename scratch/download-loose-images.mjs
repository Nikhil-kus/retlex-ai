...
const targetItems = [
  { name: 'Mulethi', query: 'mulethi roots loose white background' },
  { name: 'Soyabean Bari', query: 'soya chunks in a bowl white background' },
  { name: 'Moongfali', query: 'peanuts spilling from jute bag' },
  { name: 'Makka Poha', query: 'corn flakes makka poha loose white background' },
  { name: 'Kali Til', query: 'black sesame seeds in a white bowl wooden spoon' }
];
...
(async () => {
  ...
  for (const item of targetItems) {
    let imgUrl = await searchDuckDuckGo(item.query);
    let downloadedPath = null;
    let localFileId = `newprod_loose_${Date.now()}_${Math.floor(Math.random()*1000)}`;

    if (imgUrl) {
      console.log(`[${item.name}] ⬇️ Downloading ${imgUrl.slice(0, 30)}...`);
      ...
      if (success) {
        ...
        console.log(`[${item.name}] ❌ Download failed`);
      }
    } else {
      console.log(`[${item.name}] ❌ Not found via DDG`);
    }

    if (downloadedPath) {
        cache[item.name] = downloadedPath;
    }
    await sleep(600);
  }
  ...
  for (const shop of shopsSnap.docs) {
    for (const item of targetItems) {
        if (cache[item.name]) {
            const pSnap = await getDocs(query(collection(db, 'products'), where('shopId', '==', shop.id), where('name', '==', item.name)));
            for (const p of pSnap.docs) {
                await updateDoc(doc(db, 'products', p.id), { imageUrl: cache[item.name] });
                updated++;
            }
        }
    }
  ...
  console.log(`Updated ${updated} items with new loose images!`);
  process.exit(0);
})();
